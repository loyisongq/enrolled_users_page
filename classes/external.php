 ...
 * @since      Moodle 3.0
 */
class mod_courseuserspage_external extends external_api {
     ...
     * @since Moodle 3.0
     */
    public static function view_courseuserspage_parameters() {
        return new external_function_parameters(
            array(
                'courseuserspage' => new external_value(PARAM_INT, 'courseuserspage instance id'),
     ...
     */
    public static function view_courseuserspage($bookid, $chapterid = 0) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/courseuserspage/lib.php");
        //require_once($CFG->dirroot . "/mod/courseuserspage/locallib.php");

        $params = self::validate_parameters(self::view_book_parameters(),
                                            array(
                                                'courseuserspageid' => $bookid,
                                                'chapterid' => $chapterid
                                            ));
        $bookid = $params['courseuserspageid'];
        $chapterid = $params['chapterid'];

        $warnings = array();

        // Request and permission validation.
        $book = $DB->get_record('courseuserspage', array('id' => $bookid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($book, 'courseuserspage');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        require_capability('mod/courseuserspage:read', $context);
        ...
        if (!$chapterid) {
            $warnings[] = array(
                ...
                'itemid' => $book->id,
                'warningcode' => '1',
                'message' => get_string('nocontent', 'mod_courseuserspage')
            );
        } else {
            $chapter = $DB->get_record('courseuserspage', array('id' => $chapterid, 'bookid' => $book->id));
            $viewhidden = has_capability('mod/courseuserspage:viewhiddenchapters', $context);

            if (!$chapter or ($chapter->hidden and !$viewhidden)) {
                throw new moodle_exception('errorchapter', 'mod_courseuserspage');
            }
     ...
     * @since Moodle 3.0
     */
    public static function view_courseuserspage_returns() {
     ...
     * @since Moodle 3.0
     */
    public static function get_courseuserspage_by_courses_parameters() {
        return new external_function_parameters (
            array(
     ...
     * @since Moodle 3.0
     */
    public static function get_courseuserspage_by_courses($courseids = array()) {
        global $CFG;
        ...
        if (!empty($params['courseids'])) {
            ...
            $books = get_all_instances_in_courses("courseuserspage", $courses);
        ...
        $result = array();
        $result['courseuserspage'] = $returnedbooks;
        $result['warnings'] = $warnings;
        return $result;
    }
     ...
     * @since Moodle 3.0
     */
    public static function get_courseuserspage_by_courses_returns() {
        return new external_single_structure(
            array(
                'courseuserspage' => new external_multiple_structure(
